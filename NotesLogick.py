...
def main():
    ...
    commands = {
        ...
        "search note": lambda: print(search_notes(list_for_notes, note_text_search))
        if len(user_devided) > 1
        else print_error("write search word."),
        "edit": lambda: print(
        ...
        "add tag": lambda: print(
            add_tag(list_for_notes, list_n2[0], list_n2[1].strip())
        ),  # Додає тег
    ...
    while True:
        ...
        if user_input.lower() in commands:
            ...
            if user_input.lower() == "good bye" or user_input.lower() in [
                "close",
                "exit",
            ]:
                # commands[user_input.lower()]()
            ...
            if (
                result_text == "note"
                or result_text == "edit"
                or result_text == "update"
                or result_text == "add tag"
            ):
                list_n = user_input.split("<")
                list_n2 = list_n[1].split(">")
