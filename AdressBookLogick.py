...
class Phone(Field):
    def __init__(self, value):
        self.value = value  # Устанавливаем значение через setter
...
class Email(Field):
    ...
    def value(self, value):
        if re.match(r"[A-Za-z][\w.]*@(?:\w+\.)\w{2,}", value):
            ...
            raise ValueError("Невірний формат пошти")
...
class Birthday(Field):
    ...
    def value(self, value):
        try:
            ...
            raise ValueError("Невірний формат для дати народження")
...
class Address(Field):
    ...
    def value(self, value):
        ...
        if len(words) >= 2 and all(len(word) > 3 for word in words):
            ...
            raise ValueError("Невірний формат введення адреси")
...
class Record:
    ...
    def add_address(self, address):
        if self.address is None:
            ...
            raise ValueError("У цього запису вже існує адреса!")
    ...
    def edit_address(self, old_address, new_address):
        if self.address.value == old_address:
            ...
            raise ValueError(f"Адреса '{old_address}' не знайдена")
    ...
    def edit_email(self, old_email, new_email):
        for email in self.emails:  # ітеруюся по списку імейлів
            ...
            ):  # перевірка, чи є даний імейл тому, який ми шукали
                email.value = new_email
                return
        raise ValueError(f"Поштова адреса '{old_email}' не знайдена")
    ...
    def remove_phone(self, user_phone):
        for phone in self.phones:  # ітеруюся по списку телефонів
            ...
            ):  # перевірка, чи є даний телефон тому, який ми шукали
                self.phones.remove(phone)
                return
        raise ValueError(f"Номер телефону '{user_phone}' не знайдено")
    ...
    def __str__(self):
        attributes = [f"Ім'я контакту: {self.name.value}"]
        phones_str = "; ".join(str(phone) for phone in self.phones)
        attributes.append(f"Номери телефонів: {phones_str}")
        if self.emails:
            emails_str = "; ".join(str(email) for email in self.emails)
            attributes.append(f"Поштові адреси: {emails_str}")
        if self.address:
            attributes.append(f'Адреси: "{self.address.value}"')
        if self.birthday:
            attributes.append(f"Дні народження: {self.birthday.value.date()}")
...
class AddressBook(UserDict):
    ...
    def days_to_birthday(self, days_to_filter):
        ...
        if upcoming_birthdays:
            print(f"Збіги знайдені з такими контактами: {upcoming_birthdays}")
        else:
            print("Немає збігів.")
...
if __name__ == "__main__":
    ...
    john_record = Record(
        "John", "+380676753412", email="sdf@mail.ua", birthday="2024-02-29"
    )
